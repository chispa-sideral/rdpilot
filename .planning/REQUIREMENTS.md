...
| MCP-04 | Phase 14 / 15-03 | Complete (14-03: the [BLOCKING] `scale_to_native` bridge + precision edge/corner-tie proof is offline-complete — this requirement's own text, the coordinate-space bridge, is fully satisfied. 15-03: the live half Phase 14 deferred — whether the scaled coordinates the bridge produces actually LAND on real Windows near screen edges/corners — is now AUTHORED as a gated `tests/live_mcp_computer.rs` test in `rdpilot-mcp`, driving the real compiled binary via the same rmcp-client-subprocess pattern as PROOF-03. Maximizes 7-Zip FM (`computer` `key` `"win+up"`) so window corners coincide with screen corners, then clicks near the top-left corner (the menu bar's `"File"` item, verified via a focused UIA element) and the top-right corner (the title-bar `"Minimize"` button, verified via the window state transitioning to `"minimized"`) — both hardcoded landmarks in the advertised 1280x800 space (imported from `computer/scale.rs` via `#[path]`, never a hand-duplicated literal), never computed from a pre-click UIA query (Pitfall 3). Compiles offline and lists as ignored (`cargo test -p rdpilot-mcp --test live_mcp_computer -- --list`); live execution against the Azure VM deferred to Plan 15-06) |
...
| PROOF-04 | Phase 15 | In Progress (15-04: the PROOF-04 live-LLM capstone is now AUTHORED as a gated `tests/live_capstone.rs` test in `rdpilot-mcp`, per binding direction 1 (developer decision superseding this phase's own `15-RESEARCH.md` reqwest/Anthropic-API sketch) — the LLM driving it is the LOCAL, already-authenticated `claude` CLI in headless print mode (`claude -p "<task>" --mcp-config <rendered config>`), NEVER `ANTHROPIC_API_KEY`/`reqwest`/an SDK/a hardcoded model id. The rendered `--mcp-config` (from the committed, secret-free `tests/fixtures/capstone-mcp-config.json` template) points `claude -p` at the REAL compiled `rdpilot-mcp` binary with connection secrets passed via the server subprocess's OWN environment (D-27 `RDPILOT_HOST`/`RDPILOT_PORT`/`RDPILOT_USERNAME`/`RDPILOT_PASSWORD`) — the capstone prompt never mentions credentials, and the model never sees them. The deterministic task prompt drives BOTH halves through MCP only: `rdpilot_uia` (menu-bar read/inspect of 7-Zip File Manager, `class_name` `"7-Zip::FM"`) and `rdpilot_put`/`rdpilot_get` (file transfer), against a FIXED session name/remote name/local seed file this test itself authors. Success is asserted from TWO independent signals, never model self-report: (1) the captured `--output-format stream-json` transcript is walked (schema-tolerant recursive `tool_use` collector, since the exact CLI JSONL envelope could not be verified offline) for at least one read/inspect AND one file-transfer tool call; (2) AFTER `claude -p` exits, a SECOND, independent `rmcp` client subprocess (Pattern 2, reusing `tests/live_proof.rs`) downloads the uploaded file and compares its bytes BYTE-FOR-BYTE against the known local seed content — no re-derived trust from the model's own reported checksum, and no new dependency needed (`sha2` was considered and rejected in favor of direct byte comparison; `cargo tree`/`Cargo.toml`/`Cargo.lock` are byte-for-byte unchanged by this plan). The exact non-interactive MCP tool-approval flag for `claude -p` (`--permission-mode bypassPermissions --strict-mcp-config`, verified against the locally-installed CLI's own `--help` this session, v2.1.207) is isolated in one named constant (`PERMISSION_FLAGS`) with a documented fallback (`--dangerously-skip-permissions`), flagged TODO-for-15-08 — if headless MCP tool use turns out impossible non-interactively, the transcript assertion fails loudly by construction (zero tool calls observed), never a silent pass. Compiles offline and lists as ignored (`cargo test -p rdpilot-mcp --test live_capstone -- --list`); live execution against the Azure VM + an authenticated local `claude` deferred to Plan 15-08) |
