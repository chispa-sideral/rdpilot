...
| CLI-03 | Phase 13 | In Progress (13-03: daemon-side seam complete. 13-04: `dispatch.rs`'s `Put`/`Get` arms now call `upload_file`/`download_file` via `Registry::call`, and the pre-existing Phase-12 gap where `Connect` never set `ConnectionConfig::share_root` is fixed (new `rdpilot-config::share_root_or_default` resolver) — `put`/`get` no longer fail with a `share_root` Config error. Remaining: the actual `rdpilot` CLI binary's `put`/`get` verbs and error-taxonomy-to-exit-code mapping is 13-07) |
