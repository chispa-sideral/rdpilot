...
- [ ] **FILE-01**: User can upload a file local→remote to a named session (Rust-side share-root/validator foundation done in 10-01; sensor trigger + Session::upload_file pending later plans)
- [ ] **FILE-02**: User can download a file remote→local from a named session
- [ ] **FILE-03**: Transfer validates remote paths via canonicalization to prevent path traversal / arbitrary write (blocking security requirement) — Rust-side validator + adversarial suite done in 10-01 (D-10.2 requires BOTH sides; C# sensor-side validator pending a later plan)
